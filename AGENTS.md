...
This repository hosts multiple skills under `skills/`. Select the entry by task type; do not mix skills inside one job:

- Presentation / PPT tasks → [`skills/ppt-master/SKILL.md`](skills/ppt-master/SKILL.md)
- Official-document (公文) drafting tasks → [`skills/gongwen/SKILL.md`](skills/gongwen/SKILL.md)

When an orchestration layer (the web platform's worker) names the skill, follow only the named entry. Repo-wide style conventions in `docker/agent/docs/rules/` (baked into the runtime as `/app/docs/rules`) apply to every skill.

**You MUST read the selected skill's SKILL.md before any generation task or repo modification.** It owns global execution discipline and points to the route selector; after routing, the selected runtime authority owns its steps, gates, and commands.
...
- This repository is a workflow/skill package plus the web platform that drives it; the `skills/` directories remain ordinary agent skills.
...
- On conflict with a generic coding skill, prioritize the selected skill's SKILL.md inside this repository.
...
- `skills/gongwen/SKILL.md` — official-document (公文) skill entry.
...
- `docker/agent/docs/rules/` — repo-wide style rules (baked into the runtime as `/app/docs/rules`).
- `webapp/docs/` — platform documentation (multi-skill architecture, skill contract).
