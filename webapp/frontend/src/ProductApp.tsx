...
export function ProductApp() {
  ...
  const globalMatches = useMemo(() => {
    ...
    return [
      ...
      ...snippets.filter((snippet) => `${snippet.name}${snippet.content}`.toLowerCase().includes(query)).map((snippet) => ({ type: "预设", id: snippet.id, label: snippet.name })),
    ].slice(0, 8);
