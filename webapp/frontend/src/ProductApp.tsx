...
import { ProjectWorkbench as AssetProjectWorkbench, PromptsPage as AssetPromptsPage, TemplatesPage as AssetTemplatesPage } from "./AssetPages";
import { AdminPage as AssetAdminPage } from "./AdminPage";
import { ProjectsPage as AssetProjectsPage, ReplicaProjectCard as AssetReplicaProjectCard } from "./ProjectsPage";
import { AssetEmptyState, AsyncButton, NoticeHost, UploadButton } from "./ui";
...
export function ProductApp() {
  ...
  const replicaPageBody = nav === "templates"
    ...
    : nav === "prompts"
      ? <AssetPromptsPage snippets={snippets} query={promptQuery} setQuery={setPromptQuery} notice={notice} onUse={selectPromptPreset} onCreate={() => openPromptEditor(null)} onEdit={openPromptEditor} onDelete={(snippet) => { setTargetSnippet(snippet); setModal("prompt-delete"); }} />
  ...
  const content = activeProject ? <AssetProjectWorkbench project={activeProject} job={activeJob} workingJob={workingJob} draft={draft} setDraft={setDraft} templates={readyTemplates} selectedTemplateId={selectedTemplateId} setSelectedTemplateId={setSelectedTemplateId} artifacts={artifacts} events={events} showLogs={showLogs} setShowLogs={setShowLogs} previewSlides={previewSlides} latestPptx={latestPptx} previewIndex={previewIndex} setPreviewIndex={setPreviewIndex} isSubmitting={isSubmitting} notice={notice} onBack={() => startDraft()} onGenerate={() => void startGeneration()} onCancel={() => void cancelJob()} onUseSnippet={selectPromptPreset} snippets={snippets} /> : nav === "projects" ? <AssetProjectsPage projects={projects} jobsByProject={jobsByProject} query={projectQuery} setQuery={setProjectQuery} view={projectView} setView={setProjectView} draft={draft} setDraft={setDraft} templates={readyTemplates} selectedTemplateId={selectedTemplateId} setSelectedTemplateId={setSelectedTemplateId} snippets={snippets} isSubmitting={isSubmitting} notice={notice} onCreate={() => void beginWorkspace()} onOpen={openProject} onDelete={(project) => { setTargetProject(project); setModal("project-delete"); }} onUseSnippet={selectPromptPreset} /> : nav === "templates" ? <AssetTemplatesPage templates={templates} query={templateQuery} setQuery={setTemplateQuery} uploading={templateUploading} notice={notice} onUpload={uploadTemplate} onUse={(template) => { startDraft(); setSelectedTemplateId(template.id); }} onRename={(template) => { setTargetTemplate(template); setEditorName(template.name); setModal("template-rename"); }} onDelete={(template) => { setTargetTemplate(template); setModal("template-delete"); }} onRetry={retryTemplate} retryingTemplateId={retryingTemplateId} /> : nav === "prompts" ? <AssetPromptsPage snippets={snippets} query={promptQuery} setQuery={setPromptQuery} notice={notice} onUse={selectPromptPreset} onCreate={() => openPromptEditor(null)} onEdit={openPromptEditor} onDelete={(snippet) => { setTargetSnippet(snippet); setModal("prompt-delete"); }} /> : isPlatformAdmin ? <AssetAdminPage initialTab={route.kind === "admin" ? route.tab : "templates"} onTabChange={(tab) => navigate(`/admin/${tab}`)} templates={adminTemplates} snippets={adminSnippets} users={adminUsers} providers={adminProviders} onRefresh={loadAdminData} /> : null;

  const pageTitle = activeProject ? activeProject.title : nav === "projects" ? "我的项目" : nav === "templates" ? "模板库" : nav === "prompts" ? "常用提示词" : "管理设置";
  const pageDescription = activeProject ? "创作、生成与精修" : nav === "projects" ? "创作与管理你的演示文稿" : nav === "templates" ? "管理你的个人演示模板" : nav === "prompts" ? "沉淀高频创作与修改指令" : "管理系统模板、提示词、用户与模型";
  return <div className={`zc-shell ${sidebarCollapsed ? "is-collapsed" : ""}`}><NoticeHost message={notice} onClose={() => setNotice("")} /><button className={`zc-scrim ${sidebarOpen ? "is-visible" : ""}`} aria-label="关闭导航" onClick={() => setSidebarOpen(false)} /><aside className={`zc-sidebar ${sidebarOpen ? "is-open" : ""}`}><div className="zc-brand"><img src="/logo-128.png" alt="" /><span>智创AI助手</span></div><button className="zc-create" type="button" onClick={() => startDraft()}><Plus size={18} /><span>新建创作</span></button><nav><small>工作空间</small>{([ ["projects", "我的项目", FileStack], ["templates", "模板库", LayoutTemplate], ["prompts", "常用提示词", Zap] ] as const).map(([key, label, Icon]) => <button key={key} className={nav === key && !activeProject ? "is-active" : ""} type="button" onClick={() => { setNav(key); setActiveProjectId(null); navigate(pathForNav(key)); setSidebarOpen(false); }}><Icon size={18} /><span>{label}</span>{key === "projects" && <i>{projects.length}</i>}</button>)}</nav><div className="zc-sidebar-bottom"><div className="zc-account"><span>{user.display_name.slice(0, 1)}</span><strong>{user.display_name}</strong>{isPlatformAdmin && <button type="button" onClick={() => { setNav("admin"); setActiveProjectId(null); navigate("/admin/templates"); }} title="平台设置" aria-label="平台设置"><Settings size={16} /></button>}<button type="button" onClick={() => void logout()} title="退出登录" aria-label="退出登录"><LogOut size={16} /></button></div></div></aside><main className="zc-main"><header className="zc-topbar"><button className="zc-icon zc-mobile-menu" type="button" aria-label="打开导航" onClick={() => setSidebarOpen(true)}><Menu size={19} /></button><button className="zc-icon zc-collapse" type="button" aria-label={sidebarCollapsed ? "展开导航" : "收起导航"} onClick={() => setSidebarCollapsed((current) => !current)}>{sidebarCollapsed ? <PanelLeftOpen size={18} /> : <PanelLeftClose size={18} />}</button><div><strong>{pageTitle}</strong><span>{pageDescription}</span></div></header><div className="zc-content">{content}</div></main>{modal && <ModalFrame onClose={() => setModal(null)}>{modal === "project-delete" && <><h2>删除这个项目？</h2><p>项目中的全部生成记录和演示文稿将被删除，且无法恢复。</p><div className="zc-modal-actions"><button className="zc-secondary" onClick={() => setModal(null)}>取消</button><button className="zc-danger" onClick={() => void removeProject()}><Trash2 size={15} />确认删除</button></div></>}{renderAssetModal()}</ModalFrame>}</div>;
}
  ...
  return <div className={`zc-shell kppt-shell ${props.sidebarCollapsed ? "is-collapsed" : ""} ${props.darkMode ? "kppt-dark" : ""}`}>
    ...
    <aside className={`zc-sidebar kppt-sidebar ${props.sidebarOpen ? "is-open" : ""}`}>
      ...
      <nav className="kppt-primary-nav" aria-label="工作空间">
        <small>工作空间</small>
        {([ ["projects", "我的项目", FileStack], ["templates", "模板库", LayoutTemplate], ["prompts", "常用提示词", Zap] ] as const).map(([key, label, Icon]) => <button className={props.activeNav === key ? "is-active" : ""} key={key} type="button" onClick={() => props.onNavigate(key)}><Icon size={18} /><span>{label}</span>{key === "projects" && <i>{props.projects.length}</i>}</button>)}
      </nav>
