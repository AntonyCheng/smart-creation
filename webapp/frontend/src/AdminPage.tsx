...
function SystemPromptManager(props: { snippets: PromptSnippet[]; onRefresh: () => Promise<void> }) {
  ...
  function resetEditor(): void {
    setPromptName(""); setPromptCategory("平台"); setPromptContent(""); setEditingPromptId(null); setPromptPreset(emptySystemPromptPreset()); setPromptSlides(ensureSystemPromptSlideCount([], 8)); setGenerating(false); setOutlineNotice(""); setEditorOpen(false);
  }
  ...
  function editPrompt(snippet: PromptSnippet): void {
    ...
    setEditingPromptId(snippet.id); setPromptName(snippet.name); setPromptCategory(snippet.category); setPromptContent(snippet.content); setPromptPreset(preset); setPromptSlides(ensureSystemPromptSlideCount(normalizeSystemPromptSlides(preset), systemPromptPageCount(preset.requirements?.page_range))); setOutlineNotice(""); setEditorOpen(true);
  }
  ...
  async function savePrompt(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      const payload = { name: promptName.trim(), category: promptCategory.trim() || "平台", content: promptContent.trim(), preset: { ...promptPreset, outline: promptSlides.filter((slide) => slide.title.trim()).map((slide) => ({ ...slide, title: slide.title.trim() })) } };
...
export function AdminPage(props: { initialTab: AdminRouteTab; onTabChange: (tab: AdminRouteTab) => void; templates: Template[]; snippets: PromptSnippet[]; users: User[]; providers: Provider[]; onRefresh: () => Promise<void> }) {
  ...
  const heading = tab === "templates" ? "系统模板" : tab === "prompts" ? "系统提示词" : tab === "users" ? "用户管理" : "模型管理";
  const nav = [["templates", "系统模板", LayoutTemplate], ["prompts", "系统提示词", Zap], ["users", "用户管理", Users], ["models", "模型管理", Settings]] as const;
