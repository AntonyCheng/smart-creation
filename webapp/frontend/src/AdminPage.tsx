...
import { Edit3, Eye, Image, LayoutTemplate, LoaderCircle, Plus, Settings, Trash2, Upload, UserPlus, Users, WandSparkles, X, Zap } from "lucide-react";
import { AdminRouteTab } from "./routes";
import type { ImageProvider, ModelCatalogEntry, PromptPreset, PromptPresetSlide, Provider, PromptSnippet, Template, TemplateProgress, User } from "./appTypes";
import { IMAGE_BACKEND_CHOICES } from "./appTypes";
import { AssetEmptyState, AsyncButton, ConfirmAction, NoticeHost, PageRangeSelect, UploadButton } from "./ui";
...
export function AdminPage(props: { initialTab: AdminRouteTab; onTabChange: (tab: AdminRouteTab) => void; templates: Template[]; snippets: PromptSnippet[]; users: User[]; providers: Provider[]; modelCatalog: ModelCatalogEntry[]; imageProviders: ImageProvider[]; onRefresh: () => Promise<void> }) {
  ...
  const [modelTestVision, setModelTestVision] = useState<boolean | null>(null);
  ...
  const [defaultingVision, setDefaultingVision] = useState(false);
  ...
  const [imageProviderForm, setImageProviderForm] = useState(false);
  const [imageBackend, setImageBackend] = useState<string>(IMAGE_BACKEND_CHOICES[0]);
  ...
  const [verifyingImageProviderId, setVerifyingImageProviderId] = useState<string | null>(null);
  const [defaultingImageProviderId, setDefaultingImageProviderId] = useState<string | null>(null);
  ...
  async function createModel(event: FormEvent<HTMLFormElement>) {
    ...
    await run(async () => { await request("/api/v1/admin/providers/" + provider.id + "/models", { method: "POST", body: JSON.stringify({ model_id: modelId.trim(), display_name: modelName.trim(), is_active: true }) }); setModelProvider(null); setModelId(""); setModelName(""); setModelVerified(false); setModelTestVision(null); setModelTestMessage(""); }, "模型已创建并通过验证。");
  }
  async function testModelConnectivity(): Promise<void> {
    ...
    setModelTesting(true); setModelTestMessage("正在验证模型连通性，最长等待约 40 秒，请勿重复点击。"); setModelVerified(false); setModelTestVision(null);
    try {
      const result = await request<{ supports_vision: boolean | null }>(`/api/v1/admin/providers/${provider.id}/model-connectivity-test`, { method: "POST", body: JSON.stringify({ model_id: modelId.trim() }) });
      setModelVerified(true); setModelTestVision(result.supports_vision);
      setModelTestMessage(result.supports_vision ? "连通性测试通过，且探测到支持图片输入，可以保存并启用模型。" : "连通性测试通过，可以保存并启用模型。");
  ...
  async function setDefaultVisionModel(modelId: string | null): Promise<void> {
    if (defaultingVision) return;
    setDefaultingVision(true);
    setNotice("");
    try {
      await request(`/api/v1/admin/model-catalog/default-vision`, { method: "PATCH", body: JSON.stringify({ model_id: modelId }) });
      await props.onRefresh();
      setNotice(modelId ? "默认识图模型已更新。" : "默认识图模型已清空。");
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "默认识图模型更新失败。");
    } finally {
      setDefaultingVision(false);
    }
  }
  async function createImageProvider(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    await run(async () => {
      await request("/api/v1/admin/image-providers", {
        method: "POST",
        body: JSON.stringify({
          backend: imageBackend,
          display_name: imageDisplayName.trim(),
          api_key: imageApiKey.trim(),
          base_url_override: imageBaseUrl.trim() || null,
          model_override: imageModelOverride.trim() || null,
          is_active: true,
        }),
      });
      setImageDisplayName(""); setImageApiKey(""); setImageBaseUrl(""); setImageModelOverride(""); setImageProviderForm(false);
    }, "生图后端已创建，正在后台测试连通性（真实生成一张测试图，请稍候刷新）。");
  }
  async function verifyImageProvider(id: string): Promise<void> {
    if (verifyingImageProviderId) return;
    setVerifyingImageProviderId(id);
    setNotice("");
    try {
      await request(`/api/v1/admin/image-providers/${id}/verify`, { method: "POST" });
      await props.onRefresh();
      setNotice("已提交连通性测试，请稍候刷新查看结果。");
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "提交测试失败。");
    } finally {
      setVerifyingImageProviderId(null);
    }
  }
  async function setDefaultImageProvider(id: string | null): Promise<void> {
    if (defaultingImageProviderId) return;
    setDefaultingImageProviderId(id || "__clear__");
    setNotice("");
    try {
      await request(`/api/v1/admin/image-providers/default`, { method: "PATCH", body: JSON.stringify({ image_provider_id: id }) });
      await props.onRefresh();
      setNotice(id ? "默认生图后端已更新。" : "默认生图后端已清空。");
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "默认生图后端更新失败。");
    } finally {
      setDefaultingImageProviderId(null);
    }
  }
  ...
  const heading = tab === "templates" ? "PPT系统模板" : tab === "prompts" ? "PPT系统预设" : tab === "users" ? "用户管理" : tab === "image-providers" ? "生图后端" : "模型管理";
  const nav = [["templates", "PPT系统模板", LayoutTemplate], ["prompts", "PPT系统预设", Zap], ["users", "用户管理", Users], ["models", "模型管理", Settings], ["image-providers", "生图后端", Image]] as const;
  ...
  return <section className="zc-admin-page kppt-admin-page"><div className="zc-admin-layout"><aside aria-label="管理设置导航">{nav.map(([key, label, Icon]) => <button key={key} className={tab === key ? "is-active" : ""} type="button" onClick={() => changeTab(key)}><Icon size={17} />{label}</button>)}</aside><main><NoticeHost message={notice} onClose={() => setNotice("")} /><header className="zc-admin-section-head"><div><h1>{heading}</h1><p>{tab === "models" ? "维护模型供应商、可用模型、默认生成模型与默认识图模型（可选）。" : tab === "image-providers" ? "维护图片生成后端凭证与默认生图后端（可选，未配置时自动退化为仅搜图）。" : "维护所有用户可见的系统资源。"}</p></div>{tab === "templates" && <UploadButton className="zc-upload" icon={<Upload size={16} />} loading={busy} disabled={busy} accept=".pptx" onFiles={(files) => { const file = files[0]; if (file) void uploadTemplate(file); }}>上传系统模板</UploadButton>}{tab === "users" && <button className="zc-primary" type="button" onClick={() => setUserForm((current) => !current)}><UserPlus size={16} />{userForm ? "收起表单" : "新增用户"}</button>}{tab === "models" && <button className="zc-primary" type="button" onClick={() => setProviderForm((current) => !current)}><Plus size={16} />{providerForm ? "收起表单" : "新增供应商"}</button>}{tab === "image-providers" && <button className="zc-primary" type="button" onClick={() => setImageProviderForm((current) => !current)}><Plus size={16} />{imageProviderForm ? "收起表单" : "新增生图后端"}</button>}</header>
      ...
      {tab === "models" && <>{providerForm && <form className="kppt-admin-form kppt-provider-form" onSubmit={(event) => void createProvider(event)}><label>供应商标识<input value={providerSlug} onChange={(event) => setProviderSlug(event.target.value)} pattern="[a-z][a-z0-9_-]*" minLength={2} required /></label><label>显示名称<input value={providerName} onChange={(event) => setProviderName(event.target.value)} required /></label><label>Base URL<input type="url" value={providerUrl} onChange={(event) => setProviderUrl(event.target.value)} required /></label><label>API Key<input type="password" value={providerKey} onChange={(event) => setProviderKey(event.target.value)} required /></label><div className="kppt-admin-form-actions"><button className="zc-secondary" type="button" onClick={() => setProviderForm(false)}>取消</button><AsyncButton className="zc-primary" type="submit" loading={busy}>创建供应商</AsyncButton></div></form>}<div className="kppt-provider-list">{props.providers.length ? props.providers.map((provider) => <article className="kppt-provider-card" key={provider.id}><header><div><strong>{provider.display_name}</strong><span>{provider.slug} · {provider.base_url}</span></div><ConfirmAction title="确认删除这个供应商？" description="删除后其中的模型配置也会被移除。" disabled={busy} onConfirm={() => run(() => request<void>("/api/v1/admin/providers/" + provider.id, { method: "DELETE" }), "供应商已删除。")}><button className="zc-secondary is-danger" type="button" disabled={busy}><Trash2 size={14} />删除</button></ConfirmAction></header><p className="kppt-provider-key">API Key：{provider.api_key_hint}</p><div className="kppt-model-list"><div className="kppt-model-list-head"><strong>模型</strong><button className="zc-secondary" type="button" disabled={busy} onClick={() => { setModelProvider(provider.id); setModelId(""); setModelName(""); setModelVerified(false); setModelTestVision(null); setModelTestMessage(""); }}><Plus size={14} />新增模型</button></div>{provider.models.map((model) => <div className="kppt-model-row" key={model.id}><div><strong>{model.display_name}</strong><span>{model.model_id}</span><small>{model.is_verified ? "已验证" : "未验证"}{model.supports_vision ? " · 支持视觉" : ""}{model.last_test_error ? ` · ${model.last_test_error}` : ""}</small></div>{model.is_default && <em>默认</em>}<div className="kppt-admin-row-actions">{!model.is_verified && <AsyncButton className="zc-secondary" type="button" loading={verifyingModelId === model.id} disabled={Boolean(verifyingModelId) || !provider.is_active || !model.is_active} onClick={() => void verifyExistingModel(model.id)}>{verifyingModelId === model.id ? "验证中（最长 40 秒）" : "手动验证"}</AsyncButton>}{model.is_verified && <AsyncButton className="zc-secondary" type="button" loading={verifyingModelId === model.id} disabled={Boolean(verifyingModelId) || !provider.is_active || !model.is_active} onClick={() => void verifyExistingModel(model.id)}>{verifyingModelId === model.id ? "验证中（最长 40 秒）" : "重新验证"}</AsyncButton>}{!model.is_default && <AsyncButton className="zc-secondary" type="button" loading={defaultingModelId === model.id} disabled={busy || Boolean(verifyingModelId) || Boolean(defaultingModelId) || !model.is_active || !model.is_verified || !provider.is_active} onClick={() => void setDefaultModel(provider, model)}>设为默认</AsyncButton>}<ConfirmAction title="确认删除这个模型？" description="删除后模型将无法继续用于生成。" disabled={busy || Boolean(verifyingModelId) || Boolean(defaultingModelId)} onConfirm={() => run(() => request<void>("/api/v1/admin/models/" + model.id, { method: "DELETE" }), "模型已删除。")}><button className="zc-secondary is-danger" type="button" disabled={busy || Boolean(verifyingModelId) || Boolean(defaultingModelId)}><Trash2 size={14} />删除</button></ConfirmAction></div></div>)}{modelProvider === provider.id && <form className="kppt-admin-model-form" onSubmit={(event) => void createModel(event)}><label>模型 ID<input value={modelId} onChange={(event) => { setModelId(event.target.value); setModelVerified(false); setModelTestVision(null); setModelTestMessage(""); }} required /></label><label>显示名称<input value={modelName} onChange={(event) => setModelName(event.target.value)} required /></label>{modelTestMessage && <small className={modelVerified ? "kppt-model-test-success" : "kppt-model-test-error"}>{modelTestMessage}</small>}<div className="kppt-admin-form-actions"><AsyncButton className="zc-secondary" type="button" loading={modelTesting} onClick={() => void testModelConnectivity()} disabled={modelTesting || !modelId.trim()}>{modelTesting ? "测试中（最长 40 秒）" : "测试连通性"}</AsyncButton><button className="zc-secondary" type="button" onClick={() => setModelProvider(null)} disabled={modelTesting || busy}>取消</button><AsyncButton className="zc-primary" type="submit" loading={busy} disabled={modelTesting || !modelVerified || !modelName.trim()}>保存并启用</AsyncButton></div></form>}</div></article>) : <AssetEmptyState className="kppt-admin-empty-state" title="暂无模型供应商" description="先添加供应商，再测试并配置模型。" />}</div>{props.providers.length > 0 && <div className="kppt-vision-default"><strong>默认识图模型（可选）</strong><p>用于图片候选缩略图的人工评审；不配置时自动退化为不看图的搜图匹配。</p><select value={props.modelCatalog.find((item) => item.is_default_vision)?.model_id || ""} disabled={defaultingVision} onChange={(event) => void setDefaultVisionModel(event.target.value || null)}><option value="">不使用</option>{props.modelCatalog.filter((item) => item.supports_vision && item.is_available).map((item) => <option key={item.model_id} value={item.model_id}>{item.model_id}</option>)}</select></div>}</>}
      {tab === "image-providers" && <>{imageProviderForm && <form className="kppt-admin-form kppt-provider-form" onSubmit={(event) => void createImageProvider(event)}><label>后端<select value={imageBackend} onChange={(event) => setImageBackend(event.target.value)}>{IMAGE_BACKEND_CHOICES.map((choice) => <option key={choice} value={choice}>{choice}</option>)}</select></label><label>显示名称<input value={imageDisplayName} onChange={(event) => setImageDisplayName(event.target.value)} required /></label><label>API Key<input type="password" value={imageApiKey} onChange={(event) => setImageApiKey(event.target.value)} required /></label><label>Base URL（可选，覆盖默认地址）<input value={imageBaseUrl} onChange={(event) => setImageBaseUrl(event.target.value)} /></label><label>模型（可选，覆盖默认模型）<input value={imageModelOverride} onChange={(event) => setImageModelOverride(event.target.value)} /></label><div className="kppt-admin-form-actions"><button className="zc-secondary" type="button" onClick={() => setImageProviderForm(false)}>取消</button><AsyncButton className="zc-primary" type="submit" loading={busy} disabled={!imageDisplayName.trim() || !imageApiKey.trim()}>创建（随后后台测试）</AsyncButton></div></form>}<div className="kppt-provider-list">{props.imageProviders.length ? props.imageProviders.map((row) => <article className="kppt-provider-card" key={row.id}><header><div><strong>{row.display_name}</strong><span>{row.backend}{row.model_override ? ` · ${row.model_override}` : ""}</span></div><ConfirmAction title="确认删除这个生图后端？" description="删除后无法继续用于生成。" disabled={busy} onConfirm={() => run(() => request<void>("/api/v1/admin/image-providers/" + row.id, { method: "DELETE" }), "生图后端已删除。")}><button className="zc-secondary is-danger" type="button" disabled={busy}><Trash2 size={14} />删除</button></ConfirmAction></header><p className="kppt-provider-key">API Key：{row.api_key_hint}</p><p><small>{row.is_verified ? "已验证" : "未验证"}{row.last_test_error ? ` · ${row.last_test_error}` : ""}</small>{row.is_default && <em> · 默认</em>}</p><div className="kppt-admin-row-actions"><AsyncButton className="zc-secondary" type="button" loading={verifyingImageProviderId === row.id} disabled={Boolean(verifyingImageProviderId) || !row.is_active} onClick={() => void verifyImageProvider(row.id)}>{verifyingImageProviderId === row.id ? "提交中" : row.is_verified ? "重新测试" : "测试连通性"}</AsyncButton>{!row.is_default && <AsyncButton className="zc-secondary" type="button" loading={defaultingImageProviderId === row.id} disabled={Boolean(defaultingImageProviderId) || !row.is_active || !row.is_verified} onClick={() => void setDefaultImageProvider(row.id)}>设为默认</AsyncButton>}{row.is_default && <AsyncButton className="zc-secondary" type="button" loading={defaultingImageProviderId === "__clear__"} onClick={() => void setDefaultImageProvider(null)}>取消默认</AsyncButton>}</div></article>) : <AssetEmptyState className="kppt-admin-empty-state" title="暂无生图后端" description="不配置也能正常生成，只是没有真实照片搜不到时的 AI 生图兜底。" />}</div></>}
