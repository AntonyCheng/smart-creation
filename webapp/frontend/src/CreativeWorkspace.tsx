  ...
  return (
    <main className="zc-creative-workspace kppt-workbench-shell">
      ...
      <header className="zc-workbench-head zc-workbench-head--rich">
        ...
        <div className="zc-workbench-head-actions"><span className={`zc-status zc-status-${currentJob?.status || "draft"}`}>{props.workingJob ? (currentJob?.target_slide_number ? `正在修改第 ${currentJob.target_slide_number} 页` : "生成中") : props.job?.status === "succeeded" ? "已完成" : "创作草稿"}</span>{effectiveStage === "preview" && currentJob && <button className="zc-secondary zc-workbench-doc-edit" type="button" onClick={() => navigate(`/doc-editor/${props.project.id}?kind=${primaryKind === "docx" ? "docx" : "pptx"}`)}><Pencil size={14} />{primaryKind === "docx" ? "手动编辑" : "高级编辑"}</button>}{effectiveStage === "preview" && supportsEditor && currentJob && <a className="zc-secondary zc-workbench-manual-edit" href={editorPath(props.project.id, currentJob.id)} onClick={(event) => { event.preventDefault(); navigate(editorPath(props.project.id, currentJob.id)); }}><Pencil size={14} />SVG 编辑</a>}{effectiveStage === "preview" && latestPrimary && currentJob && <a className="zc-primary zc-workbench-export" href={artifactUrl(props.project.id, currentJob.id, latestPrimary.id)} download><Download size={14} />{primaryKind === "docx" ? "导出 DOCX" : "导出 PPTX"}</a>}</div>
      </header>
