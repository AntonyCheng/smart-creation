...
function MaterialCard(props: { materials: ProjectMaterial[]; materialUploading: boolean; onUploadMaterials: (files: File[]) => void; onDeleteMaterial: (materialId: string) => void }) {
  return <section className="zc-material-card"><header><div><Upload size={17} /><strong>创作材料</strong><span>{props.materials.length ? `已添加 ${props.materials.length} 份` : "可选"}</span></div><label className="zc-secondary zc-material-upload">{props.materialUploading ? "正在上传…" : "添加材料"}<input type="file" multiple accept=".pdf,.doc,.docx,.docm,.ppt,.pps,.pot,.pptx,.pptm,.ppsx,.ppsm,.xls,.xlsx,.xlsm,.xlsb,.odt,.ods,.odp,.rtf,.epub,.csv,.txt,.md,.markdown,.png,.jpg,.jpeg,.webp" onChange={(event) => { props.onUploadMaterials(Array.from(event.target.files || [])); event.target.value = ""; }} disabled={props.materialUploading} /></label></header>{props.materials.length ? <div className="zc-material-list">{props.materials.map((material) => { const metadata = material.metadata || {}; const parseStatus = String(metadata.parse_status || "deferred"); const isOcr = Boolean(metadata.ocr); const parseMessage = String(metadata.parse_message || (material.status === "ready" ? "已保存，生成时解析" : material.error || "处理失败")); const excerpt = String(metadata.text_excerpt || ""); const statusLabel = material.status === "processing" ? "正在解析…（扫描件 / 图片会自动 OCR 识别）" : parseStatus === "ready" ? (isOcr ? "已 OCR 识别，可参与需求和大纲" : "已解析，可参与需求和大纲") : parseStatus === "partial" ? "已 OCR 识别（置信度较低，建议核对原件）" : parseStatus === "empty" ? "未提取文本，生成时读取原文件" : parseStatus === "encrypted" ? "文件已加密，无法提取内容" : parseStatus === "failed" ? "摘要提取失败，生成时读取原文件" : parseMessage; return <div className="zc-material-item" key={material.id}><div className="zc-material-item-main"><span title={material.original_filename}>{material.original_filename}</span><small>{statusLabel}</small>{excerpt && <p title={excerpt}>摘要：{excerpt}</p>}</div><button type="button" aria-label={`删除 ${material.original_filename}`} onClick={() => props.onDeleteMaterial(material.id)}><Trash2 size={14} /></button></div>; })}</div> : <p className="zc-material-empty">添加 PDF、Office 文档、表格、文本或图片后，系统会先提取可用摘要（扫描件和图片自动 OCR 识别），再把材料交给需求、大纲和页面生成。</p>}</section>;
}
